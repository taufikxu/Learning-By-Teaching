...
def mat_to_img(w, dim_input, scale=False, colorImg=False, tile_spacing=(1, 1), tile_shape=0, save_path=None):
    if tile_shape == 0:
        rowscols = int(w.shape[0]**0.5)
        tile_shape = (rowscols, rowscols)
        w = w[0:rowscols * rowscols, :]
    imgs = tile_raster_images(X=w, img_shape=dim_input, tile_shape=tile_shape, tile_spacing=tile_spacing, scale=scale, colorImg=colorImg)
